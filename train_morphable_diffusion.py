...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-b", "--base", type=str, default='configs/facescape.yaml',)
    ...
    parser.add_argument("--load_entire_model", type=bool, default=False)
...
def load_pretrain_stable_diffusion(new_model, finetune_from, load_entire_model):
    ...
    if load_entire_model:
        ...
        for key in ckpt['state_dict']:
            if not isinstance(key, str) or 'target_encoder' in key or 'spatial_volume' not in key:
                filtered_states[key] = ckpt['state_dict'][key]
        new_model.load_state_dict(filtered_states, strict=False)
    else:
...
if __name__ == "__main__":
    ...
    if opt.finetune_from != "":
        load_pretrain_stable_diffusion(model, opt.finetune_from, opt.load_entire_model)
    model._init_multiview()
    ...
    default_modelckpt_cfg = {"target": "pytorch_lightning.callbacks.ModelCheckpoint",
                             "params": {"dirpath": ckptdir, "filename": "{epoch:06}", "verbose": True, "save_last": True, "every_n_train_steps": 1000}}
    ...
    default_modelckpt_cfg_repeat = {"target": "pytorch_lightning.callbacks.ModelCheckpoint",
                                    "params": {"dirpath": ckptdir, "filename": "{step:08}", "verbose": True, "save_last": False, "every_n_train_steps": 1000, "save_top_k": -1}}
    modelckpt_cfg_repeat = OmegaConf.merge(default_modelckpt_cfg_repeat)

    # add callback which sets up log directory
    default_callbacks_cfg = {
        "setup_callback": {
            "target": "train_morphable_diffusion.SetupCallback",
            "params": {"resume": opt.resume, "logdir": logdir, "ckptdir": ckptdir, "cfgdir": cfgdir, "config": config}
        },
        "learning_rate_logger": {
            "target": "train_morphable_diffusion.LearningRateMonitor",
            "params": {"logging_interval": "step"}
        },
        "cuda_callback": {"target": "train_morphable_diffusion.CUDACallback"},
    }
