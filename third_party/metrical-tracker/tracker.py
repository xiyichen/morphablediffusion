...
class Tracker(object):
    def __init__(self, config, device='cuda:0'):
        ...
        self.global_step = 0
        self.input_shape_path = self.config.input_shape_path
        self.target_exp_shape_path = self.config.target_exp_shape_path
    ...
    def setup_renderer(self):
        mesh_file = '../../assets/head_template_mesh.obj'
    ...
    def save_checkpoint(self, frame_id):
        ...
        shape = torch.tensor(np.load(self.input_shape_path)).float().to(self.device).reshape(self.shape.shape)
