...
parser.add_argument("--elevation_start", type=float, default=-20)
parser.add_argument("--elevation_end", type=float, default=20)
parser.add_argument("--device", type=str, default='CUDA')
parser.add_argument("--smplx_stats_path", type=str, required=True)
...
def normalize_scene(scale, uid, center):
...
def save_images(object_file: str) -> None:
    ...
    os.makedirs(os.path.join(f'{args.output_dir}/../normalization/'), exist_ok=True)
    smplx_stats = np.load(args.smplx_stats_path)
    scale = float(smplx_stats[0])
    center = smplx_stats[1:]
    reset_scene()
    # load the object
    load_object(object_file)
    # object_uid = os.path.basename(object_file).split(".")[0]
    offset_x, offset_y, offset_z = normalize_scene(scale=scale, uid=object_uid, center=center)
    np.save(f'{args.output_dir}/../normalization/{object_uid}.npy', np.array([scale, offset_x, offset_y, offset_z]))
    ...
    if args.camera_type=='random':
        K = np.array(camera.calc_matrix_camera(
